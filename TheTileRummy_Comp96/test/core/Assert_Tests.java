...
/**
 * All tests must have assert statements
 * Reading through test console output is helpful but it should not be necessary to know if test is a success
 * Only exceptions are tests for print methods ( e.g. board.print_board() )
 */
public class Assert_Tests extends TestCase {

	// Duplicate code should not exist between the two test files
	...
	/**
	 * Test if player's name is correct
	 * Doesn't work for AI players because player_id increases each time a player is created in a test
	 * So player_1 could have one player_id in one test and another id in a different test
	 */
